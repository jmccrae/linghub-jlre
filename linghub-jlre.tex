...
\begin{abstract}
    ...
    detail. We approach this problem by means of using linked data principles to
    harmonize these heterogeneous resources into a single portal called Linghub.
    We use state-of-the-art entity disambiguation techniques to combine these
    entities and provide a single interface to the user. We evaluate this
...
\begin{enumerate}
    ...
    \item Ensure that URIs resolve, in the sense that when typed in to a web browser an
...
of a web portal and thus stable URIs for resources are easily decided. It is
...
\begin{description}
    ...
    \item[Datahub.io]\footnote{\url{https://datahub.io/}}. This site is an instance of CKAN used primarily to track
...
quickly define the conversion between META-SHARE and RDF\@. %As a great many of the elements defined in the META-SHARE schema are very unique many of the elements have to be created in a new ontology, which we developed as the META-SHARE ontology, which is described in~\cite{mccrae2015ontology}. 
As it was mentioned in Section~\ref{intro}, the elements defined in the META-SHARE schema were defined as an OWL ontology for interoperability purposes, which is described in~\cite{mccrae2015ontology}. 
...
different data producers. Secondly, there are the catalogues of ELRA and LDC. The ELRA data
...
\item Datahub.io permits selecting the license in the faceted browsing they
    offer. Although nothing prevents dataset creators from declaring their own licenses, they are driven by the user interface to use one of the pre-determined license-types. This greatly reduces the license proliferation and searching by rights in Datahub is still relatively acceptable.
%<Access>
...
those labels by resources. We considered both the accuracy on the labels and
also accuracy weighted by the number of instances of that label.
For both resources, we see very high accuracy and the
...
had this result, we manually selected those senses that corresponded to
...
Linghub as a website (a screen shot is shown in
...
corpus, lexicon, tool or service; the \emph{language} the resource is applicable for;
...
all and, as seen in Figure~\ref{fig:language-constraints1}
and~\ref{fig:language-constraints2} and Table~\ref{tab:language-constraints},
this also resembles the distributon in Linghub. 
The decreasing number of requests for higher language restriction count with a
...
The high number of resources for 20 or more languages can be explained by the use of European corpora that
...
limitation that were discovered by the annotators including the lack of an
ability to set constraints on multiple property facets simultaneously, the fact
